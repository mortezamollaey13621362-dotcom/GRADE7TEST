...
export class English7App {
    constructor() {
        ...
        // ReviewManager (بعداً در openLesson ساخته می‌شود)
        ...
        // اضافه کردن event delegation برای review section
    ...
    cacheDOM() {
        ...
        // اگر review-section وجود نداشت، ایجادش کن
    ...
    // متد برای event delegation
    setupReviewEventDelegation() {
        document.addEventListener('click', (e) => {
            // بررسی کلیک روی دکمه شروع مرور
            if (e.target.closest('#startReviewBtn')) {
                console.log('🎯 Event delegation: Start review button clicked via delegation');
                e.preventDefault();
                e.stopPropagation();
                this.startReview();
                return;
            ...
            // دکمه نمایش پاسخ در کارت مرور
            if (e.target.closest('#showAnswerBtn')) {
                console.log('👁️ Event delegation: Show answer button clicked');
                e.preventDefault();
                e.stopPropagation();
            ...
            if (e.target.closest('.quality-btn')) {
                ...
                console.log('📝 Event delegation: Quality button clicked:', quality);
                e.preventDefault();
                e.stopPropagation();
            ...
            if (e.target.closest('#continueReviewBtn')) {
                console.log('🔄 Event delegation: Continue review button clicked');
                e.preventDefault();
                e.stopPropagation();
                return;
            }
            ...
            if (e.target.closest('#backToDashboardBtn')) {
                console.log('🏠 Event delegation: Back to dashboard button clicked');
                e.preventDefault();
                e.stopPropagation();
    ...
    async openLesson(lessonId) {
        ...
        try {
            ...
            // ساخت و Initialize کردن ReviewManager
            ...
            // بارگذاری داده‌های سایر ماژول‌ها
            await Promise.all([
    ...
    async switchSection(sectionId) {
        ...
        // 🧹 پاکسازی ماژول قبلی (مهم برای Speaking)
    ...
    async handleReviewSection() {
        ...
        // صرفاً داشبورد را نمایش بده
        this.renderReviewDashboard();
        
        console.log('✅ Review section rendered');
    }
    
    // اضافه کردن متد renderReviewDashboard
    ...
    // بهینه‌سازی متد createReviewSection
    createReviewSection() {
    ...
    async handleOtherSections(sectionId) {
        ...
        // مخفی کردن review-section
        if (this.dom.reviewSection) {
            this.dom.reviewSection.style.display = 'none';
        }
        
        // نمایش section-container
        if (this.dom.sectionContainer) {
            this.dom.sectionContainer.style.display = 'block';
            this.dom.sectionContainer.innerHTML = `
                <div class="loading-section">
    ...
    // ✅ اصلاح کامل متد handleSpeakingSection با await
    async handleSpeakingSection() {
        ...
        try {
            // ✅ پاکسازی کامل container
            this.dom.sectionContainer.innerHTML = '';
            this.dom.sectionContainer.style.display = 'block';
            
            // ✅ ایجاد یک container اختصاصی برای Speaking
            ...
            // ✅ Initialize کردن Speaking با await
            ...
            // ✅ Render مستقیم با container معتبر
    ...
    goToHome() {
        ...
        // 🧹 پاکسازی Speaking هنگام بازگشت به خانه
        if (this.speaking) {
            this.speaking.cleanup();
        }
    ...
    /**
     * پخش تلفظ واژگان با استفاده از AudioManager
     * @param {string} word - کلمه مورد نظر
     * @param {string} accent - لهجه (us یا uk)
     */
    async playWordAudio(word, accent = 'us') {
        // بررسی وجود AudioManager
        ...
        try {
            ...
            // فراخوانی متد playWord از AudioManager
     ...
     * @param {string} type - نوع (info, success, warning, error)
     */
    ...
    // ✅ متد showMainMenu برای دکمه Home در Speaking
    showMainMenu() {
